...
// same evening, same day: the question must stay silent
await cdp.send('Page.reload', { ignoreCache: true });
await new Promise(r => setTimeout(r, 100));
await goto();
const sameDay = await evalJs(cdp, `document.querySelectorAll('.ask').length`);
say(`  reloaded the same evening: ${sameDay} question(s) — the day gate holds`);

// now let a day pass. `opened[].day` is the real clock the ask is gated on, so
// winding it back one day is exactly "you opened these yesterday".
const moved = await evalJs(cdp, `(()=>{
  const K='roleatlas.r05a.v1', S=JSON.parse(localStorage.getItem(K));
  const d=new Date(); d.setDate(d.getDate()-1);
  const y=d.toISOString().slice(0,10); let n=0;
  Object.keys(S.keeps).forEach(s=>(S.keeps[s].opened||[]).forEach(o=>{o.day=y;n++;}));
  localStorage.setItem(K, JSON.stringify(S)); return {n:n, day:y};
})()`);
say(`  wound ${moved.n} opened role(s) back to ${moved.day} and reloaded`);
await cdp.send('Page.reload', { ignoreCache: true });
await new Promise(r => setTimeout(r, 100));
await goto();
...
const after2 = await evalJs(cdp, `(()=>({
  ...
  strip: document.querySelectorAll('.slrow').length,
  text: [...document.querySelectorAll('.slrow')].map(r=>r.innerText.replace(/\\n/g,' · ')),
  raw: localStorage.getItem('roleatlas.r05a.v1'),
}))()`);
...
const m6pass = sameDay === 0 && after.strip === 3 && after.asks.length === 1 &&
  afterAnswer === 0 && after2.asks === 0 && after2.strip === 3;
say(`M6: ${m6pass ? 'PASS' : 'FAIL'}`);

// ====================================== the shortlist leaves the page
say('\n=== the shortlist leaves the page ===');
await cdp.send('Browser.grantPermissions',
  { origin: URL.match(/^https?:\/\/[^/]+/)[0], permissions: ['clipboardReadWrite', 'clipboardSanitizedWrite'] });
await evalJs(cdp, `localStorage.clear()`);
await goto();
const names = [];
for (let i = 0; i < 3; i++) {
  names.push(await evalJs(cdp, `document.querySelectorAll('.card .cname')[${i}].innerText`));
  await click('.card .keep', i);
}
await click('#starts .linkish', 0);          // and a narrowing, so the link carries one
await new Promise(r => setTimeout(r, 400));
const share = await evalJs(cdp, `location.href`);
say(`  kept ${names.join(', ')} and narrowed; the address is now:`);
say('      ' + share);
await evalJs(cdp, `document.getElementById('copysl').click()`);
await new Promise(r => setTimeout(r, 400));
const text = await evalJs(cdp, `navigator.clipboard.readText()`).catch(() => '');
say(`  "copy my shortlist" put ${text.length} characters on the clipboard:`);
text.split('\n').slice(0, 12).forEach(l => say('      ' + l));

// the friend's browser: no localStorage of yours, only the address. about:blank
// first, because navigating to a URL that differs only in its hash changes the
// hash without reloading — and a page that never re-booted proves nothing.
await evalJs(cdp, `localStorage.clear()`);
await cdp.send('Page.navigate', { url: 'about:blank' });
await new Promise(r => setTimeout(r, 200));
await cdp.send('Page.navigate', { url: share });
for (let i = 0; i < 200; i++) {
  if (await evalJs(cdp, `document.querySelectorAll('.regrow').length>=789`).catch(() => 0)) break;
  await new Promise(r => setTimeout(r, 50));
}
const friend = await evalJs(cdp, `(()=>({
  pinned:[...document.querySelectorAll('.slrow .nm')].map(e=>e.innerText),
  said:(document.querySelector('.slrow .mt')||{innerText:''}).innerText,
  field:document.getElementById('dept').value,
  place:document.getElementById('place').value,
}))()`);
say(`  opened in a browser with the storage cleared: ${friend.pinned.length} pinned ` +
    `— ${friend.pinned.join(', ')}`);
say(`  and the narrowing came with it: field="${friend.field}" place="${friend.place}"`);
say(`  the page says: "${friend.said}"`);
const linkPass = friend.pinned.length === 3 && friend.field && friend.place &&
  text.includes('http') && text.length > 200;
say(`SHORTLIST-LEAVES-THE-PAGE: ${linkPass ? 'PASS' : 'FAIL'}`);
