...
for (let i = 0; i < RUNS; i++) {
  ...
  // the page's own stopwatch, stamped by build.py's <script> right after the
  // first card is laid out — so the number is the page's, not the poller's
  const stamp = await evalJs(cdp, `Math.round(window.__firstCardPainted||0)`).catch(() => 0);
  times.push([t, tFull, stamp]);
  console.log(`run ${i + 1}: first card ${t}ms (page's own stamp ${stamp}ms) · all 789 ${tFull}ms`);
}
const firsts = times.map(t => t[0]).sort((a, b) => a - b);
const stamps = times.map(t => t[2]).sort((a, b) => a - b);
const mid = a => a[Math.floor(a.length / 2)];
console.log(`\nnet=${which} cpu=${process.env.CPU || 1}x  median first card: ${mid(firsts)}ms ` +
  `(median self-stamp ${mid(stamps)}ms)`);
cdp.kill();
process.exit(0);
