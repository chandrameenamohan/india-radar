#!/usr/bin/env python3
"""Static server for r04-c, with gzip — because the host this would ship on
(Cloudflare Pages) serves gzipped, and M1 is a transfer-size measurement.

Run from anywhere:  python3 serve.py [port]
Without an argument it tries 8743, then 8843, then 8943.
"""
...
def main():
    ports = [int(sys.argv[1])] if len(sys.argv) > 1 else [8743, 8843, 8943]
    for port in ports:
        try:
            ...
            continue
        print(f"r04-c on http://127.0.0.1:{port}/")
        srv.serve_forever()
        return
