...
Reads design/fixture-v2/{cards,companies,build-report,first-seen}.json and
writes the two things PRODUCT-1 §7 says the build must add before the one
screen can exist:
...
def build_index(cards, full, counters):
...
def build_meta(sources, index, counters, menus):
    cards, full, report, first_seen = sources
...
def main():
    ...
    cards, full = sources[0], sources[1]
    counters = new_counters()
    index = build_index(cards, full, counters)
