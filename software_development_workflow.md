...
6. LEAVE IT CLEANER: before committing, run a quick craft pass on what you just
   ...
   structure that READ LIKE the surrounding code (e.g. /ponytail-review for
   over-engineering, /simplify for the rest). Then
...
4. LOOP_PROMPT.md — the body of one iteration, in this order:
   ...
   - Ponytail rules apply inside the loop: laddered minimalism (YAGNI → stdlib →
     native → existing dep → one line), deliberate shortcuts marked with a
     `ponytail:` comment naming the ceiling and upgrade path. NOTE: the ponytail
     hook fires per-session — verify it activates in headless `claude -p` runs
     before relying on it; if not, fold its rules into this prompt.
...
Apply the fixes that PRESERVE behavior (/ponytail-review hunts over-engineering
only; /simplify covers the rest); for judgment
...
> Periodic complements from ponytail: `/ponytail-audit` (whole-repo bloat scan,
> before a release or every few weeks) and `/ponytail-debt` (harvests the
> `ponytail:` shortcut comments the loop leaves behind into a ledger, so
> deliberate deferrals get tracked instead of rotting).
